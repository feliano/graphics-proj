...
int PERMUTATION(int coord){
  float pos = float((coord % 512));
  //pos = pos/255.0f;
  return int(texture(perm_table,pos).x * 255.0f);
}
...
//noise = texture(perm_table,0.3f).x;

if(fractal_algorithm == 0){

  // fBm
  noise = fBm(uv);
  //noise = RIDGEDMULTIFRACTAL(uv);
  //noise = HMFRACTAL(uv);
...
//if(0.0001f >= texture(perm_table,0.8f).x) color = vec3(0.8f,0.2f,0.3f);
